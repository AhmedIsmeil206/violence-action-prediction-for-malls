...
def create_detections_from_yolov8(detector, image, frame_idx, min_height=0):
    """Create detections for a given frame using YOLOv8 with ReID features."""
    detections = detector.detect(image)
    detection_list = []
    
    for det in detections:
        if isinstance(det, Detection):  # Already processed by detector
            if det.tlwh[3] < min_height:
                continue
            ...
            if bbox[3] < min_height:
                continue
            detection_list.append(Detection(bbox, confidence, np.array([])))
...
def run(sequence_dir, output_file, min_confidence, nms_max_overlap,
    ...
    # Initialize YOLOv8 detector with ReID model
    detector = YOLOv8Detector("C:/Users/Ahmed/Desktop/coding/Versions/grad i/deep_sort/human-detection/Hcrowded_project.pt", reid_model)
