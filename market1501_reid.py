...
def load_reid_model(model_path=None, device=None):
    if device is None:
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    # Set default path if none provided
    if model_path is None:
        model_path = os.path.join(os.path.dirname(__file__), 'market1501_reid.pth')
    
    # Check if model exists
    if not os.path.exists(model_path):
        raise FileNotFoundError(f"ReID model not found at {model_path}. Please train the model first.")
    
    model = ReIDFeatureExtractor(model_path=model_path).to(device)
    model.eval()
    return model
